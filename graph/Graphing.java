...
public class Graphing extends JPanel {
	...
	public Graphing(ArrayList<String> args) {
		...
		try {
			if (args.get(0).equals("python") || args.get(0).equals("c") || args.get(0).equals("java")) {
				...
				if (args.get(1).equals("merge") || args.get(1).equals("quick") || args.get(1).equals("bubble")
						|| args.get(1).equals("insertion") || args.get(1).equals("heap")) {
					int count = -1;
					for (int i = 2; i < args.size(); i++) {
				...
				} else if (args.get(1).equals("java") || args.get(1).equals("c") || args.get(1).equals("python")) {
					int count = -1;
					for (int i = 2; i < args.size(); i++) {
						if (args.get(i).equals("merge") || args.get(i).equals("quick") || args.get(i).equals("bubble")
								|| args.get(i).equals("insertion") || args.get(i).equals("heap")) {
							times.add(new ArrayList<Double>());
							count++;
							colorLabels.add(new JLabel(args.get(i)));
							colorLabels.add(new JLabel());
		...
		try {
			...
			System.out.println("Argument sizes don't match.");
			return;
		}
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < MAX_LINES + 1; i++) {
			...
			g2.drawLine(horizontalOffset, (int) (y * i) - verticalOffset, horizontalOffset + 7,
					(int) (y * i) - verticalOffset);
			g2.drawString(formatVal(((max / MAX_LINES) * (MAX_LINES + 1 - i))), 2, (int) (y * i) - 25);

			g2.setColor(gridLineColor);
			g2.drawLine(horizontalOffset, (int) (y * i) - verticalOffset, workableXarea + horizontalOffset,
					(int) (y * i) - verticalOffset);
		}
		...
		for (int i = 0; i < size; i++) {
			g2.setColor(stringColor);
			g2.drawLine((int) ((workableXarea / size) * (i)) + horizontalOffset, workableYarea - 7 + verticalOffset,
					(int) ((workableXarea / size) * (i)) + horizontalOffset, workableYarea + verticalOffset);

			g2.drawString(formatVal(xAxis.get(i)),
					(int) ((workableXarea / size) * (i)) + horizontalOffset - (horizontalOffset / 2),
					workableYarea + 15 + verticalOffset);

			g2.setColor(gridLineColor);
			g2.drawLine((int) ((workableXarea / size) * (i)) + horizontalOffset, verticalOffset,
					(int) ((workableXarea / size) * (i)) + horizontalOffset, workableYarea + verticalOffset);
		...
		g2.drawLine(horizontalOffset, workableYarea + verticalOffset, workableXarea + horizontalOffset,
				workableYarea + verticalOffset); // x
		g2.drawLine(horizontalOffset, verticalOffset, horizontalOffset, workableYarea + verticalOffset); // y

		/*
		 * Draw the coordinates. Math: x is workable graph (WIDTH - horizontalOffset) /
		 * number of elements * i, then add the horizontalOffset offset back in. y is
		 * workable graph (verticalOffset) - the ratio (value / maxValue) *
		 * (verticalOffset - verticalOffset) last part is (verticalOffset -
		 * verticalOffset) because the top line is the max value, and is at a y of
		 * verticalOffset.
		 */
		for (int list = 0; list < times.size(); list++) {
			...
			g2.fillOval((int) (((workableXarea) / size) * (size - 1)) + horizontalOffset - (POINT_SIZE / 2),
					(int) ((workableYarea + verticalOffset)
							- (times.get(list).get(times.get(list).size() - 1) / max) * (workableYarea))
							- (POINT_SIZE / 2),
					POINT_SIZE, POINT_SIZE);
		}
